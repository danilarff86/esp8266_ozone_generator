...
public:
     ...
     * if there have been changes to the buffered data (or if the buffered data has never been
     * written to the flash previously).
     *
     * @see commit()
     *
    ...
    template < typename T >
    const T&
    put( int const address, const T& v )
    {
        if ( _data && ( address >= 0 ) && ( address + sizeof( T ) <= _size ) )
        {
