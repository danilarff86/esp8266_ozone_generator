...
EEPROMClass::begin( size_t size )
{
    ...
    else
    {
        ...
        noInterrupts( );
        spi_flash_read( _sector * SPI_FLASH_SEC_SIZE + 4, reinterpret_cast< uint32_t* >( _bitmap ),
                        _bitmapSize );
        interrupts( );
 ...
 */
uint8_t
EEPROMClass::read( int const address )
{
 ...
 */
void
EEPROMClass::write( int const address, uint8_t const value )
{
    if ( address < 0 || (size_t)address >= _size )
        return;
    if ( !_data )
...
EEPROMClass::commit( )
{
    ...
    noInterrupts( );
    flashOk = spi_flash_write( _sector * SPI_FLASH_SEC_SIZE + bitmapByteUpdated + 4,
 ...
 *
 * The internal library variables & data are initialised (zeroed) but the commit() function must be
 * called to write structure (size and bitmap etc.) and any new data to the flash.
 *
 ...
 */
uint16_t
EEPROMClass::computeBitmapSize( size_t size )
{
